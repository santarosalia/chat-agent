...
- **v1.13** — retrieve 충분성 평가기를 분리한다 (ADR 0011). 검색·평가 이력을 들고 최대 3회 검색한다. `top_k`는 5 → 10 → 20. 평가기 `max_tokens` 200. 답변 LLM은 도구 없이 누적 citations로 답한다. 그래프는 prepare만 하고 JSON/SSE가 같은 경로를 쓴다.
