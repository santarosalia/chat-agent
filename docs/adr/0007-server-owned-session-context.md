...
- **`session_id`가 있으면** 요청 `messages`는 **이번 턴**(보통 마지막 `user` 하나)입니다. 서버는 저장된 user/assistant를 앞에 붙인 뒤 truncate → retrieve-evaluate → 답변 LLM을 실행합니다. 요청에 여분 히스토리가 있어도 **마지막 user만** 사용합니다.
- **`session_id`가 없으면** 기록 조회·저장을 건너뛰고, 요청 `messages`가 LLM 입력입니다(테스트 UI는 이번 user만 보내므로 멀티턴 맥락 없음).

retrieve 첫 `query`는 마지막 user 원문입니다. 추가 검색은 평가기 `missing`입니다 ([ADR 0011](./0011-retrieve-sufficiency-evaluator.md)).
...
`session_id`가 있을 때: **append 가능 여부 검사(서비스) → LangGraph(`load_history → prepare`) → retrieve-evaluate·답변(그래프 밖) → 성공 시 이번 user+assistant append(서비스)**. retrieve는 [ADR 0011](./0011-retrieve-sufficiency-evaluator.md). 그래프 소유권은 [ADR 0009](./0009-langgraph-pipeline.md).
