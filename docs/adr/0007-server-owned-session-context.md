...
`session_id`가 있을 때: **append 가능 여부 검사(서비스) → LangGraph(`load_history → prepare → llm`) → 성공 시 이번 user+assistant append(서비스)**. retrieve는 llm 노드의 도구입니다 ([ADR 0010](./0010-retrieve-as-answer-tool.md)). 그래프 소유권은 [ADR 0009](./0009-langgraph-pipeline.md).
