...
- **`group_id`**: 선택적 문자열; **생략 시 전체 코퍼스(whole-corpus) 검색** — retrieve 요청 본문에 **`group_id` 필드를 포함하지 않음**
- **`top_k`**: 선택적 숫자(요청 DTO에 남김). retrieve에 실제로 실리는 값은 평가기 루프 스케줄 **5 → 10 → 20**이다 ([ADR 0011](./0011-retrieve-sufficiency-evaluator.md)).
...
- retrieve 호출의 `top_k`는 1회 5, 2회 10, 3회 20이다. 요청 `top_k`는 이 스케줄을 바꾸지 않는다. `group_id`는 채팅 요청에서 제공된 경우에만 포함됩니다.
