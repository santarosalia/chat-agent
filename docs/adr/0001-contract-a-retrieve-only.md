...
**Status:** Accepted (Contract A retrieve 전용). **마지막 user 원문만 retrieve / 리라이트 금지**는 [ADR 0008](./0008-retrieve-query-rewrite.md)이 대체했고, 별도 리라이트는 [ADR 0010](./0010-retrieve-as-answer-tool.md)이, 도구 재검색은 [ADR 0011](./0011-retrieve-sufficiency-evaluator.md)이 대체.  
**Date:** 2026-09-10
...
**v1 retrieve 쿼리** 첫 검색은 마지막 user 원문이다. 추가 검색은 평가기 `missing`이다 — [ADR 0011](./0011-retrieve-sufficiency-evaluator.md). RAG `/v1/query` 위임은 여전히 범위 밖이다.

## Consequences

- 단일 LLM 경계: chat-agent가 assistant 응답과 `rag_used` / `citations`를 응답에서 소유합니다.
- RAG는 검색 백엔드로 유지되며, 이중 LLM 지연 또는 충돌하는 인용 의미론이 없습니다.
- retrieve 검색 품질은 시드 쿼리(마지막 user)와 평가기 `missing`에 의존합니다 ([ADR 0011](./0011-retrieve-sufficiency-evaluator.md)).
