...
- **그래프:** `load_history → prepare → retrieve → evaluate → (부족하면 retrieve) → answer`. 답변 노드는 도구 없이 누적 citations로 답한다. SSE는 같은 `invoke`에 콜백을 넘겨 `meta` 후 `stream()`. JSON `POST /chat`은 같은 그래프 `invoke`다.
